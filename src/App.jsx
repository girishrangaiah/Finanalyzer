...
export default function App() {
  ...
  const handleAnalyze = async () => {
    ...
    if (dateMode === 'single') {
      ...
      if (selectedYearNum > currentYear || (selectedYearNum === currentYear && selectedMonthIndex > currentMonthIndex)) {
        setError('Future Month/Year is not allowed.');
        return;
      }
      periodString = `${month} ${year}`;
    } else {
      if (!fromMonth || !fromYear || !toMonth || !toYear) {
        setError('Please select From and To Month and Year.');
        return;
      }
      ...
      if (fromYearNum > currentYear || (fromYearNum === currentYear && fromMonthIndex > currentMonthIndex) ||
          toYearNum > currentYear || (toYearNum === currentYear && toMonthIndex > currentMonthIndex)) {
        setError('Future Month/Year is not allowed.');
        return;
      }

      if (fromYearNum > toYearNum || (fromYearNum === toYearNum && fromMonthIndex > toMonthIndex)) {
        setError('"From" date cannot be after "To" date.');
        return;
      }
      periodString = `from ${fromMonth} ${fromYear} to ${toMonth} ${toYear}`;
    }
    ...
    try {
      const analysisResult = await analyzeFinancialDocuments(periodString, allDocs);
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
              ...
              <div className="flex space-x-4 mb-4">
                <label className="flex items-center space-x-2 cursor-pointer">
                  ...
                  />
                  <span className="text-sm font-medium text-gray-700">Single Month</span>
                </label>
                <label className="flex items-center space-x-2 cursor-pointer">
                  <input 
                    type="radio" 
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
              ...
              <p className="text-sm text-gray-500 mb-6">
                Upload up to 10 documents per category for the selected period.
                <br />
            ...
            {result ? (
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-200 h-full min-h-[400px] flex flex-col items-center justify-center p-8 text-center">
                ...
                <p className="text-gray-500 max-w-md">
                  Select a period, upload your financial documents, and click "Analyze My Finances" to get plain-language insights and actionable advice.
                </p>
              </div>
            )}
          </div>
