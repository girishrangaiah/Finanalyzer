...
export async function analyzeFinancialDocuments(period, documents) {
  // Generate a cache key based on period and document metadata
  const cacheKey = period + '|' + documents.map(d => `${d.file.name}-${d.file.size}-${d.file.lastModified}`).sort().join('|');
...
The user has selected the period: ${period}.

MANDATORY VALIDATION RULES:
1. Duplicate Detection: Check for duplicates using file name similarity, transaction overlap, salary amount repetition, identical account numbers. If duplicates are found, ignore them and include a note in your response: "This document appears to be a duplicate of an already uploaded file for this period and has not been considered."
2. Period Consistency Check: Extract and verify the statement period, payslip month, billing cycle, etc. If ANY document belongs to a different period than ${period}, STOP analysis immediately and return ONLY a validationError: "One or more uploaded documents do not match the selected period. Please upload documents for the selected period only."
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            validationError: {
              type: Type.STRING,
              description: "If Period Consistency Check fails, return the error message here. Otherwise, leave empty."
            },
            validationNote: {
              type: Type.STRING,
            ...
            actionsToTake: {
              type: Type.STRING,
              description: "Markdown text providing clear, executable actions for this period as a numbered list with headings. Each point MUST be on a separate line."
            },
            subscriptions: {
              type: Type.STRING,
